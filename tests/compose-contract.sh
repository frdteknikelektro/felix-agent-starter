...
if set(default_images.values()) != {"frdinawan/felix-agent:0.3.10"}:
...
if grep -En '0\.2\.3([^0-9]|$)|0\.3\.1([^0-9]|$)|0\.3\.3([^0-9]|$)' README.md; then
  echo "compose contract failed: README contains stale release references" >&2
  exit 1
fi

if ! grep -Eq 'frdinawan/felix-agent:0\.3\.10' README.md; then
  echo "compose contract failed: README does not document the 0.3.10 image" >&2
  exit 1
fi
