...
if set(default_images.values()) != {"frdinawan/felix-agent:0.3.5"}:
...
if ! grep -Eq 'frdinawan/felix-agent:0\.3\.5' README.md; then
  echo "compose contract failed: README does not document the 0.3.5 image" >&2
  exit 1
fi
